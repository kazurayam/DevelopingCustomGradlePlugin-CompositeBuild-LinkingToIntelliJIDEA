...
`includeBuild()` というクロージャー呼び出しに注目してほしい。これがGradleのドキュメントにおいて link:https://docs.gradle.org/current/userguide/composite_builds.html[Composite Build] と呼ばれているテクニックだ。これによって`plugin-project` プロジェクトが作成したカスタムGradleプラグインを `rehearsal-project` プロジェクトが直接に参照することができる。
